...
class Player(Entity):
    ...
    def eat(self, food):
        ...
        if self.health < self.max_health:
            ...
            scene.game.scenes['Inventory'].buttons.pop(f'use {food.name.casefold()}')
    ...
    def sell(self, item):
        ...
        scene.game.scenes['Inventory'].buttons.pop(f'sell {item.name.casefold()}')
    ...
    def gameover(self):
        """Eng: end game
            Fr: termine le jeu"""
        scene.game.scenes['Game Over'].run_scene()
...
class Monster(Entity):
    ...
    def attacking(self):
        ...
        if scene.Scene.sound and random.randint(0, 1):
            scene.Gui.hurt_sound.play()
        player.is_turn = True
